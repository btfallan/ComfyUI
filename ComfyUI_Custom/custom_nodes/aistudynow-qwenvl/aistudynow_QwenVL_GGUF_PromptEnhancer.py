...
PROMPT_CONFIG_PATH = NODE_DIR / "aistudynow_System_Prompts.json"


def load_prompt_config():
    if not PROMPT_CONFIG_PATH.exists():
        raise FileNotFoundError(f"[QwenVL] Missing aistudynow_System_Prompts.json at {PROMPT_CONFIG_PATH}")
    try:
        ...
        if not styles or not translation_prompt:
            raise ValueError("aistudynow_System_Prompts.json must include qwen_text.styles and qwen_text.translation_prompt")
        return {"styles": styles, "translation_prompt": translation_prompt}
    except Exception as exc:
        raise RuntimeError(f"[QwenVL] Failed to load aistudynow_System_Prompts.json: {exc}") from exc
...
def _close_llama_object(obj, label: str):
    ...
    for attr in ("_ctx", "_model", "_batch", "_sampler"):
        if hasattr(obj, attr):
            try:
...
class aistudynow_QwenVL_GGUF_PromptEnhancer:
    RETURN_TYPES = ("STRING",)
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "keep_model_loaded": ("BOOLEAN", {"default": False, "tooltip": "Keep GGUF model in VRAM for faster repeated runs."}),
            }
        }
    ...
    def clear(self):
        ...
        self.llm = None
        self.current_signature = None
        _close_llama_object(llm, "GGUF prompt-enhancer model")
        gc.collect()
        _release_cuda_vram()
    ...
    def process(
        ...
        try:
            ...
            if not system_prompt:
                raise ValueError("system_prompt is empty; check aistudynow_System_Prompts.json or preset selection.")
            system_prompt = (
                f"{system_prompt}\n\n"
...
NODE_CLASS_MAPPINGS = {
    "aistudynow_QwenVL_GGUF_PromptEnhancer": aistudynow_QwenVL_GGUF_PromptEnhancer,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "aistudynow_QwenVL_GGUF_PromptEnhancer": "QwenVL Prompt Enhancer (GGUF)",
}
